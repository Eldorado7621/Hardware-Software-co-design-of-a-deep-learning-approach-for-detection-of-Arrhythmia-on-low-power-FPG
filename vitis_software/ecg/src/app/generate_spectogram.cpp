...
int initDma()
{
	...
	{
		xil_printf("Error INITIALIAZING DMA \n");
		return XST_FAILURE;
	}
...
int init_cwt()
{
	...
	if(cwt_config)
	{
		...
		if(status!=XST_SUCCESS)
		{
			xil_printf("error Initializing CWT ... \n");
		}
	}
	return XST_SUCCESS;
}

int generate_spectogram(float cwt_output[SCALE][DATA_LENGTH], int &expected_outp)
{
	xil_printf(" generate spectogram function\n\r");
	static int ind=0;
	if(ind==10)
		ind=0;
	initDma();
	...
	status_tf=XAxiDma_SimpleTransfer(&axiDma,(UINTPTR)input_data[ind],(sizeof(float )*DATA_LENGTH),XAXIDMA_DMA_TO_DEVICE);
	if(status_tf!=XST_SUCCESS)
	{
		xil_printf("WRITING DATA FROM DDR TO FIFO VIA DMA FAILED  \n");
	}
	...
	expected_outp=expected_output[ind];
	ind++;
